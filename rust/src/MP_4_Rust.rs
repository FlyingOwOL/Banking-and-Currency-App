...
fn input_float() -> f64 {
    loop {
        ...
        match input.trim().parse::<f64>() {
            ...
            Err(_) => println!("Please enter a valid number (decimals allowed, e.g. 12.34)."),
        }
    }
}
...
fn register_account_name(account: &mut Account) {
    ...
    io::stdin().read_line(&mut name).expect("Failed to read");
        ...
        if name.trim().is_empty() {
            account.name = "Default User".to_string();
        } else {
            account.name = name.trim().to_string();
        }

    println!("Account Name: {} has been registered", account.name);
}
...
pub fn run() {
    ...
    let mut currency_info: [CurrencyInfo; 6] = [
        CurrencyInfo { currency: Currency::PHP, exchange_rate_from_php: 1.0, exchange_rate_to_php: 1.0, has_rate: true },
            ...
            5 => {
                loop {
                    ...
                    if currency == Currency::PHP {
                        println!("PHP exchange rate is always 1.0");
                    }
                    else {
                        for i in currency_info.iter_mut() {
                            if i.currency == currency {
                                ...
                                io::stdout().flush().unwrap(); // prompt appears first
                                i.exchange_rate_to_php = input_float();
